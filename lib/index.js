...
export function setupCommandRegex() {
    commandRegex = new RegExp(escapeRegExp(env === "production" ? prefix : testPrefix) +
        "(?<command>[\\w?]+)(?: (?<args>.+))?$");
}
let activities = [];
/**
...
function routeCommand(msg, command, args) {
    var _a;
    const { fnc, response } = (_a = commands.find((r) => typeof r.command === "string"
        ? r.command === command
        : r.command.includes(command))) !== null && _a !== void 0 ? _a : {};
    if (fnc || response) {
        (fnc !== null && fnc !== void 0 ? fnc : ((msg, args) => {
            msg.channel.send(typeof response === "string" ? response : response(args));
        }))(msg, args);
    }
}
function routeTrigger(msg) {
    var _a;
    if (!triggers.length)
        return;
    const { fnc, response } = (_a = triggers.find((t) => t.trigger.test(msg.content))) !== null && _a !== void 0 ? _a : {};
    if (fnc || response) {
        (fnc !== null && fnc !== void 0 ? fnc : ((msg) => {
            msg.channel.send(typeof response === "string" ? response : response(msg.content));
        }))(msg);
    }
}
// via MDN
