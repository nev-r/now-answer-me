...
async function routeCommand(msg, command, args) {
    ...
    if (fnc || response) {
        (fnc !== null && fnc !== void 0 ? fnc : (async (msg, args) => {
            msg.channel.send(typeof response === "string" ? response : await response(args));
        }))(msg, args);
    }
}
async function routeTrigger(msg) {
    ...
    if (fnc || response) {
        (fnc !== null && fnc !== void 0 ? fnc : (async (msg) => {
            msg.channel.send(typeof response === "string" ? response : await response(msg.content));
        }))(msg);
    }
}
// via MDN
