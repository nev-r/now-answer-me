...
async function routeCommand(msg, command, args) {
    ...
    if (response)
        makeResponseFunction(response)(msg, args);
    if (fnc)
        fnc(msg, args);
}
async function routeTrigger(msg) {
    ...
    if (response)
        makeResponseFunction(response)(msg, msg.content);
    if (fnc)
        fnc(msg, msg.content);
}
function makeResponseFunction(response) {
    return async (msg, args) => {
        const msgContent = typeof response === "function" ? await response(args) : response;
